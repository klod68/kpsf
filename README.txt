...
How did it start?
The first version of this project was in 2006, when I was thinking to write a Persistence subsystem using the requirements settled
...
conditions for a fully-strength persistent sub-system.
...
uses a database mapper (Fowler), when the client make persistent object that combines persistence with domain data. The other use metadata-
based mappers that reads from an XML file the mapping from object properties to columns or values for the storage mechanism.


Which persistent mechanism it is supported?
The project has an implementation for SQL Server, but the main project is an abstraction that can be implemented for other mechanism.
...
Yes. The main methods Store, Change, Retrieve and Destroy  of the PersistentServer class has overloading versions that accept multiple
objects.

Is there support for transactions?
No. I believe that transactions are a persistence mechanism constraint, not an object oriented persistence service feature. 
Some mechanism (text files, for example) doesn't permit transactions, like relational databases. In these cases, when you need
...
extracted from multiple objects, and configure an atomic transaction, for saving in multiple tables.

When my domain classes changes or I adde new classes, do I need to change the persistence service code?
...
The project implement the Ambler's object identifier patthern, with an ObjectIdentifer abstract class, and an implementation class
called OID.
...
But, this persistence service has no knowledge of the environment it runs.
...
The SQL Server implementation doesn't specific code with versioning in mind.
The code can be used with any of the SQL Server versions.
...
as you wish with your persistence mechanism in mind. You can if you want, to extend the code of your implementation to support the
language that use your contact mechanism.
